...
//        System.out.println("Actual Term: " + log.get(log.size() - 1).term());
//
//        assertFalse(log.isEmpty(), "Log should contain the new entry.");
//        assertEquals("System started", log.get(log.size() - 1).message());
//        assertEquals(currentTerm, log.get(log.size() - 1).term(),
...
//                .anyMatch(e -> e.message().equals("Should not be committed"));
//
